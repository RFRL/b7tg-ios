...
private final class PasscodeOptionsControllerArguments {
    ...
    let setDuressCode: () -> Void
    let removeDuressCode: () -> Void

    init(turnPasscodeOff: @escaping () -> Void, changePasscode: @escaping () -> Void, changePasscodeTimeout: @escaping () -> Void, changeTouchId: @escaping (Bool) -> Void, setDuressCode: @escaping () -> Void, removeDuressCode: @escaping () -> Void) {
        ...
        self.setDuressCode = setDuressCode
        self.removeDuressCode = removeDuressCode
    }
}
...
private enum PasscodeOptionsEntry: ItemListNodeEntry {
    ...
    case duressSet(PresentationTheme, String, String)
    case duressRemove(PresentationTheme, String)
    case duressInfo(PresentationTheme, String)
    ...
    var section: ItemListSectionId {
        switch self {
            ...
            case .duressSet, .duressRemove, .duressInfo:
                return PasscodeOptionsSection.duress.rawValue
        }
    }
    ...
    static func ==(lhs: PasscodeOptionsEntry, rhs: PasscodeOptionsEntry) -> Bool {
        switch lhs {
            ...
            case let .duressSet(lhsTheme, lhsText, lhsValue):
                if case let .duressSet(rhsTheme, rhsText, rhsValue) = rhs, lhsTheme === rhsTheme, lhsText == rhsText, lhsValue == rhsValue {
                    return true
                } else {
                    return false
                }
            case let .duressRemove(lhsTheme, lhsText):
                if case let .duressRemove(rhsTheme, rhsText) = rhs, lhsTheme === rhsTheme, lhsText == rhsText {
                    return true
                } else {
                    return false
                }
            case let .duressInfo(lhsTheme, lhsText):
                if case let .duressInfo(rhsTheme, rhsText) = rhs, lhsTheme === rhsTheme, lhsText == rhsText {
                    return true
                } else {
                    return false
                }
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        switch self {
            ...
            case let .duressSet(_, title, value):
                return ItemListDisclosureItem(presentationData: presentationData, systemStyle: .glass, title: title, label: value, sectionId: self.section, style: .blocks, action: {
                    arguments.setDuressCode()
                })
            case let .duressRemove(_, title):
                return ItemListActionItem(presentationData: presentationData, systemStyle: .glass, title: title, kind: .destructive, alignment: .natural, sectionId: self.section, style: .blocks, action: {
                    arguments.removeDuressCode()
                })
            case let .duressInfo(_, text):
                return ItemListTextItem(presentationData: presentationData, text: .plain(text), sectionId: self.section)
        }
    }
}
...
private func passcodeOptionsControllerEntries(presentationData: PresentationData, state: PasscodeOptionsControllerState, passcodeOptionsData: PasscodeOptionsData, currentAccountId: Int64) -> [PasscodeOptionsEntry] {
    ...
    switch passcodeOptionsData.accessChallenge {
        ...
        case .numericalPassword, .plaintextPassword:
            ...
            let existingDuressCode = loadDuressMappings().first(where: { $0.accountId == currentAccountId })
            entries.append(.duressSet(presentationData.theme, "Duress Code for This Account", existingDuressCode != nil ? "Configured" : "Not Set"))
            if existingDuressCode != nil {
                entries.append(.duressRemove(presentationData.theme, "Remove Duress Code"))
            }
            entries.append(.duressInfo(presentationData.theme, "Entering this code on the lock screen instead of your real passcode opens this account, while any other accounts stay completely hidden."))
    }
...
func passcodeOptionsController(context: AccountContext, focusOnItemTag: PasscodeOptionsEntryTag? = nil) -> ViewController {
    ...
    let arguments = PasscodeOptionsControllerArguments(turnPasscodeOff: {
        ...
        let setupController = PasscodeSetupController(context: context, mode: .setup(change: false, .digits6))
        setupController.complete = { passcode, numerical in
            setDuressMapping(code: passcode, accountId: context.account.id.int64)
            let _ = (passcodeOptionsDataPromise.get() |> take(1)).start(next: { [weak passcodeOptionsDataPromise] data in
                passcodeOptionsDataPromise?.set(.single(data))
            })
            popControllerImpl?()
        }
        pushControllerImpl?(setupController)
    }, removeDuressCode: {
        removeDuressMapping(accountId: context.account.id.int64)
        let _ = (passcodeOptionsDataPromise.get() |> take(1)).start(next: { [weak passcodeOptionsDataPromise] data in
            passcodeOptionsDataPromise?.set(.single(data))
        })
    })
    ...
    let signal = combineLatest(context.sharedContext.presentationData, statePromise.get(), passcodeOptionsDataPromise.get()) |> deliverOnMainQueue
        |> map { presentationData, state, passcodeOptionsData -> (ItemListControllerState, (ItemListNodeState, Any)) in
            ...
            let listState = ItemListNodeState(presentationData: ItemListPresentationData(presentationData), entries: passcodeOptionsControllerEntries(presentationData: presentationData, state: state, passcodeOptionsData: passcodeOptionsData, currentAccountId: context.account.id.int64), style: .blocks, ensureVisibleItemTag: focusOnItemTag, emptyStateItem: nil, animateChanges: false)
